...
export default function AdminTeachers({ teachers: initial, allCourses }) {
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">{teachers.length} teacher{teachers.length !== 1 ? "s" : ""} registered</p>
          </div>
        </div>

        {/* Add teacher */}
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5 mb-6">
          <h2 className="text-sm font-semibold text-amber-200 mb-3">👨‍🏫 Add Teacher</h2>
          <p className="text-xs text-slate-400 mb-4">
            The person must already have an Alambana account (signed up via the student portal).
            Enter their email to grant teacher access.
          </p>
        ...
        {teachers.length === 0 ? (
          <div className="rounded-xl border border-white/10 p-10 text-center text-slate-400 text-sm">
            No teachers yet. Add a teacher using the form above.
          </div>
        ) : (
...
export async function getServerSideProps({ req }) {
  ...
  const { data: profiles } = await supabaseAdmin.from("profiles").select("user_id, display_name, role").eq("role", "teacher");
  ...
  return { props: { teachers, allCourses: allCourses ?? [] } };
}
