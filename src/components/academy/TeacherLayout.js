import { useState, useEffect } from "react";
import Head from "next/head";
import Link from "next/link";
import { useRouter } from "next/router";

const NAV = [
  { href: "/academy/teacher/dashboard",  icon: "📊", label: "Dashboard" },
  { href: "/academy/teacher/sessions",   icon: "📡", label: "Live Sessions" },
  { href: "/academy/teacher/students",   icon: "👥", label: "My Students" },
  { href: "/academy/teacher/analytics",  icon: "📈", label: "Analytics" },
];

export default function TeacherLayout({ children, title = "Teacher Portal" }) {
  const router = useRouter();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [teacherEmail, setTeacherEmail] = useState(null);

  useEffect(() => {
    import("@/lib/supabase").then(({ supabase }) => {
      supabase.auth.getSession().then(({ data }) => {
        if (data.session) setTeacherEmail(data.session.user.email);
      });
    });
  }, []);

  async function handleLogout() {
    const { supabase } = await import("@/lib/supabase");
    await supabase.auth.signOut();
    router.push("/academy/teacher/login");
  }

  const isActive = (href) =>
    href === "/academy/teacher/dashboard"
      ? router.pathname === "/academy/teacher/dashboard"
      : router.pathname.startsWith(href);

  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div className="px-4 py-5 border-b border-white/10">
        <Link href="/academy/teacher/dashboard" className="block">
          <p className="text-xs font-bold uppercase tracking-widest text-amber-300">Alambana EduTech</p>
          <p className="text-[10px] text-slate-500 mt-0.5">Teacher Portal</p>
        </Link>
      </div>

      {/* Nav */}
      <nav className="flex-1 py-4 overflow-y-auto">
        {NAV.map((n) => (
          <Link
            key={n.href}
            href={n.href}
            onClick={() => setSidebarOpen(false)}
            className={`flex items-center gap-3 px-4 py-2.5 text-sm transition mx-2 rounded-xl mb-0.5 ${
              isActive(n.href)
                ? "bg-amber-500/10 border border-amber-500/20 text-amber-200 font-medium"
                : "text-slate-400 hover:text-slate-100 hover:bg-white/[0.04]"
            }`}
          >
            <span className="text-base">{n.icon}</span>
            <span>{n.label}</span>
          </Link>
        ))}

        {/* My Courses — dynamic links filled by dashboard, just a section header */}
        <div className="mt-3 mx-4 mb-1">
          <p className="text-[10px] text-slate-600 uppercase tracking-widest font-semibold">My Courses</p>
        </div>
      </nav>

      {/* Footer */}
      <div className="px-4 py-4 border-t border-white/10 flex flex-col gap-2">
        {teacherEmail && (
          <div className="flex items-center gap-2 px-1 mb-1">
            <div className="w-6 h-6 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white text-[10px] font-black shrink-0">
              {teacherEmail[0].toUpperCase()}
            </div>
            <span className="text-[10px] text-slate-400 truncate">{teacherEmail}</span>
          </div>
        )}
        <Link href="/academy" className="flex items-center gap-2 text-xs text-slate-500 hover:text-amber-200 transition">
          <span>↗</span> View Academy
        </Link>
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 text-xs text-slate-500 hover:text-red-300 transition text-left"
        >
          <span>⎋</span> Sign out
        </button>
      </div>
    </div>
  );

  return (
    <>
      <Head><title>{title} | Teacher Portal</title></Head>
      <div className="min-h-screen bg-slate-950 text-slate-100 flex">

        {/* Desktop sidebar */}
        <aside className="hidden md:flex flex-col w-52 lg:w-56 shrink-0 border-r border-white/10 bg-slate-900/40 sticky top-0 h-screen">
          <SidebarContent />
        </aside>

        {/* Mobile sidebar overlay */}
        {sidebarOpen && (
          <div className="md:hidden fixed inset-0 z-50 flex">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />
            <aside className="relative z-10 w-56 bg-slate-900 border-r border-white/10 flex flex-col">
              <SidebarContent />
            </aside>
          </div>
        )}

        {/* Main area */}
        <div className="flex-1 flex flex-col min-w-0">
          <header className="md:hidden sticky top-0 z-40 border-b border-white/10 bg-slate-900/60 backdrop-blur-xl px-4 py-3 flex items-center justify-between">
            <button onClick={() => setSidebarOpen(true)} className="text-slate-400 hover:text-slate-100 transition text-lg">☰</button>
            <span className="text-sm font-semibold">{title}</span>
            <div className="w-6" />
          </header>
          <main className="flex-1 min-w-0">{children}</main>
        </div>
      </div>
    </>
  );
}
